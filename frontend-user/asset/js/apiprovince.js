...
document.addEventListener("DOMContentLoaded", function () {
	...
	provinceSelect.addEventListener("change", () => {
		...
		if (provinceCode) {
			axios
				.get(`${API_BASE}/p/${provinceCode}?depth=2`)
				.then((res) => {
					console.log(
						"Đã load danh sách quận huyện:",
	...
	districtSelect.addEventListener("change", () => {
		...
		if (districtCode) {
			axios
				.get(`${API_BASE}/d/${districtCode}?depth=2`)
				.then((res) => {
					console.log(
						"Đã load danh sách phường xã:",
						res.data.wards.length,
						"phường/xã"
					);
